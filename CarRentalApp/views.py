...
def delete_booking(request, id):
    # Retrieve the booking data or return a 404 page if not found
    book_data = get_object_or_404(car_booking, id=id)

    # Check if booking data is found
    if book_data:
        # Retrieve the corresponding car data
        car_data = car_details.objects.filter(name=book_data.car_name).first()

        # Update car availability and save changes
        if car_data:
            car_data.is_available = True
            car_data.save()
    ...
    # Redirect to the total_booking page
    return redirect('total_booking')
